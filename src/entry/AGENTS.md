...
- Use `ui-bind.js` metadata helpers for simple setting persistence: `data-sc-setting` for plain inputs and `data-sc-slider-setting`/`data-sc-partner-input` for slider-chip pairs. Add `data-sc-slider-max-setting` when a slider's maximum depends on another setting; keep handlers with workflow side effects explicit.
