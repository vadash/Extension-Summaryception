...
- **Temporal metadata:** Snippets use `sourceRange` plus hour-level `currentDateTime`; chronology/promotion anchors render as `[msgs X-Y; current T]` with no `turnRange` backfill, and generated anchors strip repeated leading old or current-style `[msgs ...]` prefixes from snippet prose.
...
- `maxL0SourceTokens` (default 8000, slider 2k-16k) is the user-facing hard ceiling for the raw source text sent to any single L0 summary call. Planners target `minSummaryBudget` when safe, clamp under the ceiling, and keep slight overshoot as one partition rather than splitting mid-target. A single indivisible assistant turn above the ceiling stays as one partition. `minSummaryBudget` is capped at `maxL0SourceTokens` (cannot collect more than the cap).
- Cache auto-flush executes one all-or-nothing atomic L0 transaction via `summarizeAtomicLayer0Partitions` in `summarizer-batch.js`: all partitions share the same frozen memory context, store layers and `summarizedUpTo` are not mutated mid-flight, and any abort/stale/validation failure discards every pending snippet and leaves chat/memory untouched. Post-save persistence failures (ghosting, injection) also roll back the in-memory store and re-save metadata so the atomicity contract holds past `saveChatStore()`. Shared by `commitLayer0Snippet` and `commitAtomicLayer0Snippets` via `runCommitPersistence`. Use `partition.sourceStartIdx`/`sourceEndIdx` for partition ranges, not just assistant turn indices.
