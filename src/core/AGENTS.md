...
- **Temporal metadata:** New snippets may carry `sourceRange` plus hour-level `timelineStart`/`timelineEnd`/`currentDateTime`; chronology/promotion anchors must use this metadata only, with no legacy `turnRange` backfill, and generated anchors must strip any leading legacy `[msgs ...]` prefix from snippet prose.
