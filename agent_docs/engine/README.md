...
- `layer0-compression.js` validates Layer 0 sections independently after cleanup and integrity validation. `layer0SummaryTokenTarget` is the `[NARRATIVE]` soft target with a 1.5x model-facing maximum plus a narrow 1.65x repair ceiling, avoiding retries on provider near misses. Rolling `[STATE]` uses fixed 200-token soft target and 300-token model-facing maximum; states through 360 tokens get deterministic snapshot compaction before an LLM repair. Provider output caps are separate; never derive them from `layer0SummaryTokenTarget`.
- `src/core/token-budget/` pre-counts source-relative sizing for L0/regen calls and builds a `<summaryception_source_budget>` prompt block. `budget-hint-builder.js` renders bounds through `applySafetyGap` (`safety-gap.js`, ratio 0.9), so the numbers the model sees are ~90% of the real validation bounds while ceilings (300 / 330 / 360) stay unchanged — overshoot into the gap still passes validation. `structural-constraints.js` adds source-derived caps (STATE lines `min(keys||7,7)`; NARRATIVE sentences `clamp(ceil(tokens/500),3,5)`) framed with equal weight to the token budget. `repair-feedback-adapter.js` appends a line/sentence-count line to repair feedback alongside the token diagnostics. The hint is pre-computed in `summarizer-pipeline.js` `attachBudgetHint` and threaded via `metadata.budgetHint` so `appendLayer0PromptConstraints` stays sync; sizing numbers live only in the hint, the `l0_constraints` block keeps qualitative rules.
