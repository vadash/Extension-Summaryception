...
- Layers 1+ promote older snippets. Narrative is compressed by LLM; source-size compression accounting counts source narratives only, while rolling states remain contextual input. Snapshot sources use the final snapshot in the promoted span and are not merged or carried into a complete remaining snapshot.
